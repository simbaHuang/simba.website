...
$.fn.slider = function(opts){
	...
	imgsMove = function(){
		...
		if(opts.moveStyle == "slide"){
			$thsLink.eq(count).stop().animate({
				"left": 0},
				opts.moveTime, function() {
				var countR = (count == imgSize - 1 ? -1 : count) + 1;
				$thsLink.eq(countR).css({"left":"100%"});
			});
			$thsLink.eq(count-1).stop().animate({
				"left": "-100%"},
				opts.moveTime);
		}else if(opts.moveStyle == "fade"){
			$thsLink.css({"left": 0});
			$thsLink.stop().fadeOut(opts.moveTime).eq(count).fadeIn(opts.moveTime);
		}
		tagsMove();
		startSlider();
	},
